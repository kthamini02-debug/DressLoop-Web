...
exports.getRequestsByDonation = getRequestsByDonation;
...
async function createRequest(req, res) {
    ...
    try {
        ...
        const checkDuplicate = await db.query('SELECT id FROM requests WHERE donation_id = $1 AND ngo_id = $2', [donation_id, req.user.id]);
        if (checkDuplicate.rows.length > 0) {
            return res.status(400).json({ error: 'You have already sent a request for this donation.' });
        }
        // 4. Create request
        const requestId = (0, uuid_1.v4)();
        await db.query('INSERT INTO requests (id, donation_id, ngo_id, status) VALUES ($1, $2, $3, $4)', [requestId, donation_id, req.user.id, 'pending']);
        ...
        if (donation.status === 'available') {
            ...
            await db.query('INSERT INTO donation_status_history (id, donation_id, old_status, new_status) VALUES ($1, $2, $3, $4)', [(0, uuid_1.v4)(), donation_id, 'available', 'requested']);
        }
        // 6. Notify Donor
        await (0, notification_1.createNotification)(donation.donor_id, 'New Donation Request', `NGO "${ngo.organization_name}" has requested your donation: "${donation.title}".`);
...
async function getMyRequests(req, res) {
    ...
    try {
        const result = await db.query(`SELECT r.id as request_id, r.status as request_status, r.created_at as request_date,
...
async function getRequestsByDonation(req, res) {
...
async function updateRequestStatus(req, res) {
    ...
    try {
        ...
        const reqRes = await db.query(`SELECT r.*, d.donor_id, d.title as donation_title, d.status as donation_status,
       ...
       WHERE r.id = $1`, [id]);
        ...
        if (['accept', 'reject', 'complete'].includes(action)) {
            ...
            if (action === 'accept') {
                ...
                await db.query('UPDATE requests SET status = \'rejected\' WHERE donation_id = $1 AND id != $2', [request.donation_id, id]);
                // Update donation status to accepted
                await db.query('UPDATE donations SET status = \'accepted\' WHERE id = $1', [request.donation_id]);
                // Log status history
                await db.query('INSERT INTO donation_status_history (id, donation_id, old_status, new_status) VALUES ($1, $2, $3, $4)', [(0, uuid_1.v4)(), request.donation_id, request.donation_status, 'accepted']);
                // Notify accepted NGO
                await (0, notification_1.createNotification)(request.ngo_id, 'Donation Request Accepted', `Your request for "${request.donation_title}" has been accepted! You can now chat with the donor.`);
                // Notify rejected NGOs
                const otherReqs = await db.query('SELECT ngo_id FROM requests WHERE donation_id = $1 AND id != $2', [request.donation_id, id]);
                for (const reqRow of otherReqs.rows) {
                    await (0, notification_1.createNotification)(reqRow.ngo_id, 'Request Declined', `The donor accepted another request for "${request.donation_title}".`);
                }
            ...
            if (action === 'reject') {
                ...
                // Notify NGO
                await (0, notification_1.createNotification)(request.ngo_id, 'Request Declined', `Your request for "${request.donation_title}" has been declined by the donor.`);
                // Check if there are other pending requests.
                const pendingRes = await db.query('SELECT id FROM requests WHERE donation_id = $1 AND status = \'pending\'', [request.donation_id]);
                ...
                if (pendingRes.rows.length === 0) {
                    ...
                    await db.query('INSERT INTO donation_status_history (id, donation_id, old_status, new_status) VALUES ($1, $2, $3, $4)', [(0, uuid_1.v4)(), request.donation_id, request.donation_status, 'available']);
                }
            ...
            if (action === 'complete') {
                ...
                // Log history
                await db.query('INSERT INTO donation_status_history (id, donation_id, old_status, new_status) VALUES ($1, $2, $3, $4)', [(0, uuid_1.v4)(), request.donation_id, request.donation_status, 'completed']);
                // Notify NGO
                await (0, notification_1.createNotification)(request.ngo_id, 'Donation Completed', `The donor confirmed collection of "${request.donation_title}". Thank you for your support!`);
        ...
        if (action === 'collect') {
            ...
            // Log history
            await db.query('INSERT INTO donation_status_history (id, donation_id, old_status, new_status) VALUES ($1, $2, $3, $4)', [(0, uuid_1.v4)(), request.donation_id, request.donation_status, 'collected']);
            // Notify Donor
            await (0, notification_1.createNotification)(request.donor_id, 'Donation Collected', `NGO "${request.ngo_name}" has marked "${request.donation_title}" as collected. Please confirm collection.`);
