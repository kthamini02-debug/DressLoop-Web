...
export async function getDashboardStats(req: Request, res: Response) {
  ...
  try {
    ...
    userStatsRes.rows.forEach((row: any) => {
      if (row.role in userStats) {
        userStats[row.role as keyof typeof userStats] = parseInt(row.count, 10);
      }
    });

    // 2. NGO Stats
    const ngoStatsRes = await db.query(
      'SELECT approval_status, COUNT(*) as count FROM ngos GROUP BY approval_status'
    );
    const ngoStats = { pending: 0, approved: 0, rejected: 0 };
    ngoStatsRes.rows.forEach((row: any) => {
      if (row.approval_status in ngoStats) {
        ngoStats[row.approval_status as keyof typeof ngoStats] = parseInt(row.count, 10);
      }
    });

    // 3. Donation Stats
    const donationStatsRes = await db.query(
    ...
    donationStatsRes.rows.forEach((row: any) => {
      if (row.status in donationStats) {
        donationStats[row.status as keyof typeof donationStats] = parseInt(row.count, 10);
      }
    });

    // 4. Recent Donations
    const recentDonationsRes = await db.query(
      `SELECT d.id, d.title, d.category, d.status, d.created_at, u.name as donor_name 
       FROM donations d 
    ...
    // 5. Recent NGOs
    const recentNgosRes = await db.query(
      `SELECT n.id, n.organization_name, n.approval_status, n.created_at, u.email 
       FROM ngos n 
    ...
    const activeChatsRes = await db.query(
      "SELECT COUNT(DISTINCT CASE WHEN sender_id < receiver_id THEN sender_id || '_' || receiver_id ELSE receiver_id || '_' || sender_id END) AS count FROM messages"
    );
    ...
    const categoryDistribution = categoryDistributionRes.rows.map((row: any) => ({
    ...
    requestStatusRes.rows.forEach((row: any) => {
      if (row.status in requestStatusStats) {
        requestStatusStats[row.status as keyof typeof requestStatusStats] = parseInt(row.count, 10);
      }
    });
...
export async function getNgos(req: Request, res: Response) {
  if (!req.user || req.user.role !== 'admin') {
  ...
  try {
    const result = await db.query(
      `SELECT n.*, u.name as admin_contact_name, u.email 
       FROM ngos n 
       JOIN users u ON n.id = u.id 
...
export async function verifyNgo(req: Request, res: Response) {
  if (!req.user || req.user.role !== 'admin') {
    return res.status(403).json({ error: 'Access denied.' });
  }
  ...
  try {
    ...
    if (status === 'approved') {
      await createNotification(
        id,
        'Account Approved',
        'Congratulations! Your NGO registration has been approved by the administrator. You can now browse and request donations.'
      );
    } else {
      await createNotification(
        id,
        'Account Rejected',
        'Your NGO registration has been rejected by the administrator. Please contact support if you believe this is an error.'
      );
    }
