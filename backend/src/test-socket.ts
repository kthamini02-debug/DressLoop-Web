...
async function runSocketTests() {
  ...
  try {
    ...
    const donorToken = jwt.sign(
      { id: donorUser.id, name: donorUser.name, email: donorUser.email, role: donorUser.role },
      JWT_SECRET,
      { expiresIn: '1h' }
    );
    const ngoToken = jwt.sign(
      { id: ngoUser.id, name: ngoUser.name, email: ngoUser.email, role: ngoUser.role },
      JWT_SECRET,
      { expiresIn: '1h' }
    );
    ...
    await db.query(
      `INSERT INTO donations (id, donor_id, title, description, category, gender, age_group, size, quantity, condition, images, status) 
       VALUES ($1, $2, 'Chat Test Coat', 'Test description', 'Coats', 'Unisex', 'Adult', 'M', 1, 'Good', $3, 'accepted')`,
      [donationId, donorUser.id, JSON.stringify(['/uploads/test.jpg'])]
    );

    const requestId = uuidv4();
    await db.query(
      'INSERT INTO requests (id, donation_id, ngo_id, status) VALUES ($1, $2, $3, \'accepted\')',
      [requestId, donationId, ngoUser.id]
    );
    console.log('✓ DB state prepared (donor and NGO have an accepted request).');

    // 3. Connect Donor socket
    console.log('\n🔌 Connecting Donor Socket client...');
    const donorSocket = ioClient(backendUrl, {
      auth: { token: donorToken },
      transports: ['websocket'],
    });

    let donorOnlineUsers: string[] = [];
    let donorReceivedMessages: any[] = [];
    let donorTypingEvents: any[] = [];
    let donorNotifications: any[] = [];

    donorSocket.on('connect', () => {
      console.log('⚡ Donor socket connected!');
    });

    donorSocket.on('online_users_list', (list: string[]) => {
    ...
    const ngoSocket = ioClient(backendUrl, {
      auth: { token: ngoToken },
      transports: ['websocket'],
    });

    let ngoOnlineUsers: string[] = [];
    let ngoReceivedMessages: any[] = [];
    let ngoTypingEvents: any[] = [];
    let ngoNotifications: any[] = [];

    ngoSocket.on('connect', () => {
      console.log('⚡ NGO socket connected!');
    });

    ngoSocket.on('online_users_list', (list: string[]) => {
      ngoOnlineUsers = list;
    });
